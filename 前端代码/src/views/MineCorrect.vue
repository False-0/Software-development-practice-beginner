...
import { deleteCorrectRecord, fetchAllCorrectRecords } from '../api/correct'
...
async function batchDelete() {
  if (!selectedIds.value.length) {
    showFailToast('请先选择记录')
    return
  }
  try {
    await showConfirmDialog({
      title: '删除记录',
      message: `确定删除选中的 ${selectedIds.value.length} 条吗？`,
    })
  } catch {
    return
  }
  ...
  try {
    await Promise.all(ids.map((id) => deleteCorrectRecord(id)))
  ...
  <div class="records-page">
    <div class="shell">
      ...
      <van-checkbox-group v-else v-model="selectedIds" class="list-wrap">
        <div v-if="!list.length" class="empty-tip">暂无做题记录</div>

        <div
          v-for="item in list"
          :key="item.correctId ?? item.id"
          class="q-card"
        >
